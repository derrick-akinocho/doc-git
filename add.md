...
--sparse
Autoriser la mise à jour des entrées d’index en dehors du cône d’extraction clairsemée. Normalement, git add refuse de mettre à jour les entrées d’index dont les chemins ne rentrent pas dans le cône d’extraction clairsemée, car ces fichiers pourraient être supprimés de l’arbre de travail sans avertissement. Voir git-sparse-checkout[1] pour plus de détails.
...
-u
--update
Mettre à jour l’index sur les seuls fichiers déjà présents et correspondant à <spec-de-chemin>. Cela retire ou modifie les entrées d’index pour correspondre à la copie de travail, mais n’ajoute pas de fichier.

Si aucun <spec-de-chemin> n’est spécifié quand l’option -u est utilisée, tous les fichiers suivis dans la totalité de l’arbre de travail sont mis à jour (les versions anciennes de Git limitaient la mise à jour au répertoire courant et ses sous-répertoires).

-A
--all
--no-ignore-removal
Mettre à jour l’index non seulement pour tous les fichiers de l’arbre de travail correspondant à <spec-de-chemin> mais aussi pour toutes les entrées existant déjà dans l’index. Ceci ajoute, modifie et retire des entrées d’index pour correspondre à la copie de travail.

Si aucun <spec-de-chemin> n’est spécifié quand l’option -A est utilisée, tous les fichiers de l’arbre de travail sont mis à jour (les versions anciennes de Git utilisaient le répertoire courant et ses sous-répertoires).

--no-all
--ignore-removal
Mettre à jour l’index en y ajoutant les nouveaux fichiers qui sont inconnus et les fichiers modifiés dans la copie de travail, mais ignore les fichiers qui ont été effacés de la copie de travail. Cette option ne fait rien quand aucun <spec-de-chemin> n’est utilisé.
